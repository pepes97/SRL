...
class SvevaDatasetBERT_GCN(Dataset):
    ...
    """
      Return: 
        - Initializes the "self.sentence" and "self.labels" lists. Each element of "self.sentence" contains a list of 
          ...
          and a list containing a single predicate. If the sentence contains more than one predicate, it will be repeated as 
          many times as there are predicates. Each element of "self.labels" contains a list of roles associated with that particular 
  ...
  def encode_labels(self, sentence, offset, vocab):

    """
      Args:
        - sentence (list):  list containing the labels of the sentence
        - offset (list of tuple): offset of words caused by BERT tokenization, because a word can be divided into sub-tokens. 
                                  I have to go to align the labels based on these offsets to have a clear correspondence 
                                  between the words and the labels
        - vocab (dict): vocabulary of labels

      Return:
        - data: list of encoding labels aligned according to offset. labels no longer have 
  ...
  def encode_pred_lemma_BERT(self, sentence, offset, vocab):
  ...
  def build_word2idx(self):
    ...
    for elem in tqdm(self.data):
      ...
      for w in words:
        ...
        ids_to_tokens += self.tokenizer.tokenize(w)

      sub_word_idx = self.tokenizer.build_inputs_with_special_tokens(sub_word_idx) 


      word_pretrained = self.encode_pred_lemma_BERT(words, word_offset, self.w_vocab)

      role = out["roles"]
      labels = self.encode_labels(role,word_offset, self.l_vocab)

      # lemmas

      lemmas = inp["lemmas"]
      lemma = self.encode_pred_lemma_BERT(lemmas, word_offset, self.lemma_vocab)
      ...
      predic = inp["predicate"]
      pred = self.encode_pred_lemma_BERT(predic, word_offset, self.pred_vocab)      
